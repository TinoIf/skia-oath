...
func update_state():
	# PERBAIKAN: Jika sedang HIT, ATTACK, atau DASH, jangan ganti state dulu!
	if current_state in [State.ATTACK, State.DASH, State.HIT]: 
		return
...
func handle_state_logic(delta):
	...
	var direction = Input.get_axis("left_mov", "right_mov")
	# Saat HIT, kita biarkan momentum knockback bekerja (tidak bisa input jalan)
	if current_state not in [State.ATTACK, State.HIT]:
		...
		# Perlahan berhenti saat terkena hit
		velocity.x = move_toward(velocity.x, 0, friction * delta)
...
func _on_animation_player_animation_finished(anim_name: StringName) -> void:
	# PERBAIKAN: Masukkan "hit" agar kembali ke IDLE setelah animasi selesai
...
func take_damage(jumlah: float, source_pos: Vector2 = Vector2.ZERO):
	...
	if current_hp <= 0:
		...
		# OPSIONAL: Tambahkan knockback pada Player agar menjauh dari monster
